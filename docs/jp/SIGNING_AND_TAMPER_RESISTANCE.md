...
重要点:
- 署名対象は `audit_summary` だけではなく、**ポリシー参照情報・commit 紐付け・event_root を含む署名付き証明全体**です。
...
5. （ローカルに session state がある場合）raw 監査ログ（`audit_exec.jsonl`, `audit_workspace_write.jsonl`）から `event_root` を再計算し、以下と照合
   - `event_root.json`
   - `attestation.json` の `integrity.event_root` / `integrity.event_count`
6. `require-pass` 条件に基づく pass/fail 判定
...
- 生イベント全件を常に署名付き証明に含めなくても、
  - 「このサマリはこのイベント列から作られた」という整合性アンカーを持てる
- 後からイベント列が差し替えられた場合の検知根拠になる

### 5.2 PoC の位置づけ

PoC では `event_root` 自体は署名対象の署名付き証明に含まれるため、
...
加えて、ローカルに session の raw 監査ログが残っている環境では、`attested verify` が `event_root` を再計算して照合するため、

- `audit_exec.jsonl`
- `audit_workspace_write.jsonl`

の事後改ざんも検知できます（`AUDIT_LOG_INTEGRITY_MISMATCH`）。
...
- session 監査結果の結論（pass/fail）の改ざん
  - 署名付き証明の署名で保護される
- ローカル raw 監査ログの事後改ざん（session state が残っている workspace 上で verify する場合）
  - `event_root` 再計算照合で検出される
...
- 監査主体側の実行時証明（collector/host の健全性証明）
