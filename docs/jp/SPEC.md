...
本ドキュメントは attested の PoC 仕様を定義する。PoC は Docker 上の開発コンテナを対象とし、ホスト側監査（LSM/eBPF）で得た観測結果をコミットに紐付け、署名付きアテステーションとして出力する。

本仕様の中心は「AI Agent 検出専用」ではなく、ポリシー（禁止実行体 / 許可書き込み実行体）に基づいて開発セッションのプロセス実行とワークスペース書き込みを検証する、一般的な attestation 基盤である。AI Agent 禁止はその代表的なポリシー適用例として扱う。
...
- Attestor: 監査サマリをポリシー評価し、アテステーションを生成して署名するCLI/サービス
