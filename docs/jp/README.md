...
**SessionAttested** は、開発セッション中に「どのプロセスが実行され、どの実行体がワークスペースへ書き込んだか」をホスト側監査（LSM/eBPF）で観測し、その結果をコミットに紐付けてアテステーション（署名付き証明）として出力するプロジェクトです。

本プロジェクトの仕組みは **ポリシーベースの開発セッション証跡 / attestation 基盤** として利用できます。  
...
flowchart LR
  subgraph Auditor["監査主体管理下ホスト (Linux)"]
    ...
    A5["検証結果出力<br>ATTESTED / ATTESTED_SUMMARY"]
  end
...
- **何が出力されるか:** 署名付き attestation と verify 結果（`ATTESTED`, `ATTESTED_SUMMARY`）
...
- 監査主体が管理するホスト上で実行される、被監査主体のコンテナ環境（PoCでは Docker）において、
  ...
  をホスト側で監査する
- コミット（commit SHA）に対して、監査結果を要約したアテステーションを生成し、署名する
- 生成されたアテステーションは GitHub Artifact として配布できる形式を想定する（PoCではローカル生成も可）
...
```json
{"pid":318728,"comm":"node","filename":"/home/dev/.vscode-server/.../codex"}
{"pid":318944,"comm":"codex","filename":"/home/dev/.vscode-server/.../codex"}
```
...
```json
{"pid":318944,"comm":"codex","filename":"/workspace/src/create_by_codex.txt","op":"open_write"}
{"pid":318944,"comm":"codex","filename":"/workspace/src/edited_by_codex.txt","op":"open_write"}
```
...
- 監査結果を `attestation` / `verify` / `ATTESTED_SUMMARY` として残せる
...
- 「この実装は監査対象セッション内で作業した」ことを commit と attestation で紐付けて提示する
...
- 受託側/被監査主体は、管理下セッションでの作業結果に attestation を添付して提出する
- 納品物レビューとは別に、作業プロセス面のチェック材料として利用する

### 4. 教育・試験・トレーニング環境での補助監査

- 演習/試験時に「禁止ツール（生成AI、外部取得ツール等）」をポリシー化
- 提出時に attestation を添付させ、ルール違反の一次チェックを自動化する
...
- 署名付き attestation をアーカイブし、後日監査時の証跡として扱う
...
- SessionAttestedでの対応/解決方針:
  ...
  - `attestation` / `verify` により開発フロー側で検証可能な形式へ変換
...
- SessionAttestedでの対応/解決方針:
  ...
  - `event_root` + 署名で attestation 改ざん検知を可能にする
...
- PoCは Docker、最終的に K8s へ拡張可能な形式（session/policy/attestation）を維持する

## リポジトリ構成

- [`cmd/`](../../cmd/) : CLI エントリポイント（`attested`）
- [`internal/`](../../internal/) : コア実装（collector, attest/verify, state, policy, docker 連携など）
- [`schemas/`](../../schemas/) : JSON Schema（attestation / audit event）
...
- [`ATTESTATION_FLOW.md`](ATTESTATION_FLOW.md) : 監査主体/被監査主体の役割とコマンドベースの attestation フロー
...
- [`POC_EXAMPLE_CODEX_SESSION.md`](POC_EXAMPLE_CODEX_SESSION.md) : Codex 書き込みセッションの PoC 実例（fail case）
