  ...
  "workspace_files": [
    {
      ...
      "write_count": 1,
      "comms": ["codex"],
      "writers": [{"sha256": "sha256:...", "path_hint": "/home/dev/.vscode-server/.../codex"}]
    }
  ],
...
- file-level write attribution review (`workspace_files`)

`workspace_files` is useful when you want to understand:
...
## 10. `.attest_run/reports/sessions/<SESSION_ID>/session_correlation.json`

Session-scoped correlation artifact generated by `verify --write-result`.

This JSON captures the correlation results used by the WebUI for:
...
- reproducible session correlation review outside the WebUI
- keeping WebUI output aligned with persisted JSON artifacts
- commit-file / writer / forbidden-lineage explanation in reports or CI exports

Operational note:

- WebUI prefers this artifact as the source of truth for session correlation cards
- if missing, WebUI may fall back to deriving correlations from raw logs and current artifacts (for backward compatibility)
